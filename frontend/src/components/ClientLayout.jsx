...
function ClientLayout({ children }) {
  ...
  const navItems = [
    ...
    { path: '/client/events', icon: '🎉', label: 'Mes événements' },
    { path: '/client/services', icon: '🛎️', label: 'Services' },
    { path: '/client/payments', icon: '💳', label: 'Paiements' },
    { path: '/client/invitations', icon: '✉️', label: 'Invitations' },
    { path: '/client/messages', icon: '💬', label: 'Messages' },
    { path: '/client/gallery', icon: '🖼️', label: 'Galerie' },
    { path: '/client/profile', icon: '👤', label: 'Profil' },
    { path: '/client/settings', icon: '⚙️', label: 'Paramètres' },
  ];
        ...
        <nav className="flex-1 p-4 flex flex-col gap-1 overflow-y-auto">
          {navItems.map((item) => (
            <Link key={item.path} to={item.path} className={linkClass(item.path)} onClick={() => setSidebarOpen(false)}>
              ...
              {item.path === '/client/messages' && unread > 0 && (
                <span className="ml-auto bg-accent-500 text-primary-950 text-xs font-bold px-2 py-0.5 rounded-full">{unread}</span>
              )}
            </Link>
          ))}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-gray-200 px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 z-30">
          ...
          <h1 className="font-heading font-bold text-primary-900 text-lg hidden md:block">
            {navItems.find(i => i.path === location.pathname)?.label || 'Espace Client'}
          </h1>
          <div className="flex items-center gap-3">
            <Link to="/client/messages" className="relative text-gray-500 hover:text-primary-700">
              🔔
