...
function ProviderMessages() {
  ...
  return (
    <ProviderLayout>
      ...
      {loading ? <p className="text-gray-400">Chargement...</p> : notifications.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {notifications.map(n => (
            <div key={n.id} className={`bg-white rounded-2xl shadow-card border p-5 flex gap-4 items-start ${!n.lu ? 'border-primary-200 bg-primary-50/30' : 'border-gray-100'}`}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-lg ${!n.lu ? 'bg-primary-700 text-white' : 'bg-gray-100 text-gray-500'}`}>🔔</div>
              ...
              {!n.lu && (
                <button onClick={() => handleMarkRead(n.id)} className="text-primary-700 text-xs font-medium hover:underline shrink-0">Marquer lu</button>
              )}
            </div>
          ))}
        </div>
